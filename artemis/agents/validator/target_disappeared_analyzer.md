...
1. **Analyze**: Compare the two states provided to you:
   ...
   - Identify why the target element disappeared (use the **Diagnosis Tips** below).
...
- **`ask_explorer(query, context_feedback)`**: Call this visual parsing subagent to search for the visual icon/pattern on the current screen if the target is absent from the standard "UI Element List" but you see it is visually present on the screenshot.
