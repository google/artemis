...
class ModelRouter:
    ...
    def __init__(self, default_model: str = "gemini-3.7-flash"):
        ...
        self._role_routes: dict[str, ModelEndpoint] = {}
        self._fallback_routes: dict[str, list[ModelEndpoint]] = {}
    ...
    def set_role_route(
        ...
        fallbacks: list[ModelEndpoint] | None = None,
    ) -> None:
        """Configures or overrides a route for a specific agent role."""
        if isinstance(endpoint, str):
            endpoint = ModelEndpoint(model_name=endpoint)
        self._role_routes[role.lower()] = endpoint
        if fallbacks is not None:
            self._fallback_routes[role.lower()] = fallbacks

    def get_endpoint(self, role: str) -> ModelEndpoint:
        """Retrieves the configured primary model endpoint for a role."""
        return self._role_routes.get(role.lower(), self.default_endpoint)

    def get_fallbacks(self, role: str) -> list[ModelEndpoint]:
        """Retrieves fallback endpoints for a role."""
        return self._fallback_routes.get(role.lower(), [])

    def instantiate_model(self, endpoint: ModelEndpoint) -> BaseChatModel:
        """Instantiates a concrete LangChain chat model via the unified ModelFactory."""
        return ModelFactory.create_model(endpoint)

    def create_chat_model(self, role: str) -> BaseChatModel:
        """Creates a chat model with configured fallbacks for the specified role."""
        primary_endpoint = self.get_endpoint(role)
        primary_model = self.instantiate_model(primary_endpoint)

        fallbacks = self.get_fallbacks(role)
        if fallbacks:
            fallback_models = [self.instantiate_model(ep) for ep in fallbacks]
