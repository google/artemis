...
def publish_startup_progress(stage: str, message: str, **details: Any) -> None:
    """Publish one startup milestone, ignoring unavailable UI/IPC endpoints."""
    session_id = os.getenv("ARTEMIS_SESSION_ID") or os.getenv(
        "ARTEMIS_CLOUD_SESSION_ID"
    )
    ipc_port = read_ipc_port()
    if not session_id or not ipc_port:
        return
