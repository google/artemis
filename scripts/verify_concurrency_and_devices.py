...
async def run_experiment_1_single_device_relay(real_serial: str):
    ...
    client = ArtemisClient(device_serial=real_serial, default_profile="flash", concurrency_mode="per_device")
    print(f"Client Init: device_serial='{client.device_serial}', concurrency_mode='{client.concurrency_mode}'")
    ...
    async def execute_task_2():
        ...
        print(f"[{now_str()}] ▶ [Task 2] Submitted concurrently (detects device occupied by Task 1, entering FIFO queue...)")
    ...
    print(f"  Task 1 runtime interval: [{task_1_start:.3f}s ~ {task_1_end:.3f}s] (elapsed {task_1_end - task_1_start:.3f}s)")
    ...
    assert task_2_end - task_1_start >= (task_1_end - task_1_start) + 0.1, "Tasks failed to execute in sequential queue!"
    print("  ✅ Verification successful: Task 2 queued properly and relayed immediately after Task 1 released the lock!")
...
async def run_experiment_2_multi_device_parallel(real_serial: str, emu_serial: str):
    ...
    client_a = ArtemisClient(device_serial=real_serial, default_profile="flash", concurrency_mode="per_device")
    client_b = ArtemisClient(device_serial=emu_serial, default_profile="flash", concurrency_mode="per_device")
    ...
    async def run_on_device_a():
        ...
        a_end = time.time()
        print(f"[{now_str()}] ◀ [Device A Task] Completed! Elapsed: {a_end - a_start:.3f}s, Device: {res.device_id}")
        return res
    ...
    async def run_on_device_b():
        ...
        b_end = time.time()
        print(f"[{now_str()}] ◀ [Device B Task] Completed! Elapsed: {b_end - b_start:.3f}s, Device: {res.device_id}")
        return res
    ...
    print(f"  Device A execution interval: {a_start:.3f}s ~ {a_end:.3f}s (elapsed {a_end - a_start:.3f}s)")
    print(f"  Device B execution interval: {b_start:.3f}s ~ {b_end:.3f}s (elapsed {b_end - b_start:.3f}s)")
    print(f"  Total concurrent duration: {t_total:.3f}s (sum of sequential durations: {(a_end - a_start) + (b_end - b_start):.3f}s)")
    ...
    assert t_total < ((a_end - a_start) + (b_end - b_start) * 0.95), "Total duration should be less than sum of both tasks!"
...
async def run_experiment_3_global_concurrency_serialization(real_serial: str, emu_serial: str):
    ...
    async def run_global_task_2():
        ...
        print(f"[{now_str()}] ▶ [Global Task 2] Submitted to emulator ({emu_serial}), entering queue due to global limit...")
        res = await client_b.run("Global task 2", max_turns=3)
        t2_end = time.time()
        print(f"[{now_str()}] ◀ [Global Task 2] Relayed completion! Total time (including queue): {t2_end - t2_submitted:.3f}s")
        return res

    res1, res2 = await asyncio.gather(run_global_task_1(), run_global_task_2())
    print(f"  Global Task 1 interval: {t1_submitted:.3f}s ~ {t1_end:.3f}s (elapsed {t1_end - t1_submitted:.3f}s)")
    print(f"  Global Task 2 submit time: {t2_submitted:.3f}s")
    print(f"  Global Task 2 end time: {t2_end:.3f}s (waited for Task 1 completion before executing)")
    assert t2_end > t1_end, "Global Task 2 did not complete after Global Task 1!"
    assert (t2_end - t1_end) >= 0.08, "Global Task 2 did not execute sequentially after Task 1 released global lock!"
    print("  ✅ Verification successful: Under global mode, strictly 1 task executed sequentially across different devices!")
...
async def run_experiment_4_parameters_and_utilities(real_serial: str, emu_serial: str):
    ...
    print(f"2. Chain switch to physical device: client.set_device('{real_serial}') -> '{client.device_serial}'")
    assert client.device_serial == real_serial

    # Attribute switch
    client.device_serial = emu_serial
    print(f"3. Property assignment to emulator: client.device_serial = '{emu_serial}' -> '{client.device_id}'")
    assert client.device_id == emu_serial

    # Dynamically change concurrency mode
    client.set_concurrency_mode(ConcurrencyMode.GLOBAL)
    print(f"4. Chain change concurrency mode: client.concurrency_mode = '{client.concurrency_mode}'")
    ...
    for d in devices:
        print(f"   • [{d.serial}] State: {d.state}, Model: {d.model}, IsEmulator: {d.is_emulator}, Busy: {d.is_busy}")
    ...
    print(f"   Task result: Status={result.status}, Device={result.device_id}, Trace={result.trace_id[:8]}...")
...
async def main():
    ...
    for d in devices:
        print(f"  • Serial: {d.serial} | Model: {d.model} | State: {d.state} | IsEmulator: {d.is_emulator}")
    print("=" * 76)

    import argparse

    parser = argparse.ArgumentParser(description="Live experiment demonstrating Artemis concurrency modes and device targeting.")
    parser.add_argument("--phone", default=os.environ.get("ARTEMIS_PHONE_SERIAL"), help="Target physical phone serial number")
    parser.add_argument("--emulator", default=os.environ.get("ARTEMIS_EMULATOR_SERIAL"), help="Target Android emulator serial number")
    ...
    if real_serial not in serials or emu_serial not in serials:
        print(f"Error: Both physical device '{real_serial}' and emulator '{emu_serial}' must be connected for full experiment.")
    ...
    print_header("🎉 All experiments completed successfully! All concurrency and device targeting verified!")
