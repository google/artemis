...
export class AgentStreamComponent {
  ...
  // Set to track expanded state of action cards
  ...
  public isActionExpanded(cardId: string): boolean {
    return this.expandedActionCards().has(cardId);
  }

  public toggleActionExpanded(cardId: string, event?: Event): void {
    if (event) {
      event.stopPropagation();
    }
  ...
  public isFailureAnalyzerActionTool(tool: any): boolean {
    return isFailureAnalyzerActionTool(tool);
  }
  ...
  public hasVisibleContent(block: any): boolean {
    ...
    const hasVisibleTools = Boolean(block.data?.generic_tools) && block.data.generic_tools.some((t: any) => this.shouldShowTool(t, block.data) || (t.type === 'llm_call' && t.status === 'failed'));
    const hasAndroidActions = Boolean(block.data?.action_taken) && this.isAndroidAction(block.data.action_taken);
  ...
  public isItemRunning(item: any, block: any): boolean {
    ...
    if (item.data.status === 'success' || item.data.status === 'failed') return false;
    if (item.data.action === 'report_task_status' || item.data?.action_taken?.action === 'report_task_status') return false;
  ...
  public isBlockRunning(block: any): boolean {
    ...
    if (block.data.action_taken?.action === 'report_task_status' || block.data?.status === 'completed' || block.data?.status === 'failed') return false;
    if (block.data.isCompleted === false && this.isCurrentSessionActive()) return true;
    if (this.isCurrentSessionActive()) {
      const blocks = this.consolidatedBlocks();
      const lastBlock = blocks.length > 0 ? blocks[blocks.length - 1] : null;
