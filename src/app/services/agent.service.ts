...
export class AgentService {
  ...
  public sessions = computed(() => {
    ...
    const hasRunningInRaw = raw.some(s => s.status === 'running' || (runId && s.session_id === runId));

    if (status === 'running' && goal && runId && !hasRunningInRaw) {
      const activeRunningSession: Session = {
        session_id: runId,
        initial_goal: goal,
      ...
      return [activeRunningSession, ...pending, ...raw];
    }
      ...
      this.eventSource?.addEventListener(eventType, (event: MessageEvent) => {
        try {
          ...
          if (eventType === 'trace_recorded' && parsedData) {
            ...
            if (['save_note', 'read_note', 'update_note', 'append_note', 'list_notes'].includes(trName.toLowerCase())) {
    ...
    this.http.get<any>('/api/status').subscribe({
      next: (data) => {
        if (data && data.status) {
          ...
          // Auto-select the session if a new active session starts and user is not browsing an old historical session
          if (data.status === 'running' && data.session_id && data.session_id !== oldRunningSessionId) {
            const currentId = this.currentSessionId();
            if (!currentId || currentId === oldRunningSessionId) {
