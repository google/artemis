...
export function drawActionCoordinatesOnOverlay(
  ...
  actions.forEach(act => {
    ...
    // 1. Check separate start and end coordinate objects
    ...
    // 2. Check 4-element coordinate array in coordinates, target, action, gesture, etc.
    ...
    const rawCoords = normCoords ||
                      ...
                      extractNumbersFromCoordinateValue(act.sequence) ||
                      extractNumbersFromCoordinateValue(args.sequence) ||
    ...
    // Multi-point click sequence
    const seq = act.sequence || args.sequence || (Array.isArray(act.target) && Array.isArray(act.target[0]) ? act.target : null);
    if (seq && Array.isArray(seq)) {
      let drawnAny = false;
      seq.forEach((pt: any) => {
        const p = extractNumbersFromCoordinateValue(pt);
        if (p && p.length === 2) {
          createPointMarker(p[0], p[1], actionName, naturalWidth, naturalHeight, overlay, isNorm);
          drawnAny = true;
        }
      });
      if (drawnAny) return;
    ...
    // 3. Check for directional swipe/scroll/drag gestures ONLY when action is swipe-like
    if (isSwipeLike) {
    ...
    // 4. Default tap/click coordinate fallback
