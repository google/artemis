    ...
    if (log.type === 'llm_stream') {
      ...
      if (existingIndex > -1) {
        ...
        newTools.forEach((nt: any) => {
          if (!mergedTools.some(et => et.trace_id && nt.trace_id && et.trace_id === nt.trace_id)) {
            mergedTools.push(nt);
          }
        });
      ...
      const isTool = log.data.type === 'tool';
      ...
      if (existingIndex > -1) {
        ...
        const genericTools = stepData.generic_tools || [];
        if (!genericTools.some((t: any) => t.trace_id === log.data.trace_id)) {
          const updatedTools = [...genericTools, log.data];
          blocks[existingIndex] = {
            ...blocks[existingIndex],
            data: {
              ...stepData,
              generic_tools: updatedTools
            }
          };
        }
      } else {
...
export function getSortedStepEvents(
  ...
  if (stepData.action_taken && isAndroidAction(stepData.action_taken)) {
...
export function compileSessionSummary(logs: any[]): string {
  ...
  // Check for goal completed / checker response
  for (let i = logs.length - 1; i >= 0; i--) {
    ...
    if (log.type === 'step_recorded' && log.data?.status === 'completed') {
...
export function checkPlanningLoader(
  ...
  for (let i = logs.length - 1; i >= Math.max(0, logs.length - 3); i--) {
    ...
    if (log.type === 'step_recorded' || log.type === 'step_updated') {
      ...
      if (act && (act.action === 'report_task_status' || act === 'report_task_status')) {
        return false;
      }
